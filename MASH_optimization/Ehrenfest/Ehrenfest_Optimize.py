...
ehrenfest_df = pd.read_csv("/u/dem/kebl6911/Part-II/MASH_optimization/Ehrenfest/wignormalisedpop.dat", delimiter=" " , names=column_names)
ehrenfest_df = ehrenfest_df[(ehrenfest_df.index % 5 == 0) | (ehrenfest_df.index == len(ehrenfest_df.index) - 1)]
...
for t1 in t1s:
    try:
        ...
        optimize_init = Optimizer(start_to_t1, eq_pop, no_states, initial_kappas=kappas)
        rms, optimized_kappas= optimize_init.run()
        save_kappas.append(optimized_kappas)
        kappas = optimized_kappas
...
np.savetxt(f"scan_0_5000_wig3_resid1.dat", save_resid1, delimiter = "\t")
np.savetxt(f"scan_0_5000_wig3_resid2.dat", save_resid2, delimiter = "\t")
np.savetxt(f"scan_0_5000_wig3_resid3.dat", save_resid3, delimiter = "\t")
np.savetxt(f"scan_0_5000_wig3_neghalf.dat", save_neg, delimiter = "\t")
np.savetxt(f"scan_0_5000_wig3_kappas.dat", save_kappas, delimiter = "\t")
