...
#site_df = site_df[(site_df.index % 20 == 0) | (site_df.index == len(site_df.index) - 1)]
...
for index, start_time in enumerate(start_time_values, start=1):
    ...
    residual_sum_val, optimized_data = optimize(site_data, initial_guess_kappa, initial_guess_p, eq_pop,full_time,site_full)
    residual_sum.append(residual_sum_val)

    #kappa_saved.append(index, least_squares_result)
    np.savetxt(f"Start_window_no_negatives_1/{start_time}_{index}.dat", optimized_data, delimiter="\t")



residual_data = np.column_stack((start_time_values,residual_sum))
np.savetxt(f"Residual_nonegpop_1.dat", residual_data,delimiter="\t")
