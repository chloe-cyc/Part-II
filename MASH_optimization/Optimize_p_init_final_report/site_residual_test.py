...
for index, start_time in enumerate(start_time_values, start=1):
    ...
    residual_sum_val, optimized_data, prev_kap_1 = optimize(site_data, prev_kap, initial_guess_p, eq_pop, full_time,t_one) #t_one,t_two, full_time)
    prev_kap = prev_kap_1
    residual_sum.append(residual_sum_val)

    #kappa_saved.append(index, least_squares_result)
    np.savetxt(f"new_site4/{start_time}_{index}.dat", optimized_data, delimiter="\t")

residual_data = np.column_stack((start_time_values,residual_sum))
np.savetxt(f"site_residual4.dat", residual_data,delimiter="\t")