...
site_df = site_df[(site_df.index % 3 == 0) | (site_df.index == len(site_df.index) - 1)]
site_eq = site_df.values
full_time = site_df["Time"].values
site_df = site_df[site_df["Time"]<=2000]
...
start_time_values = start_time_values[:-30]
...
for index, start_time in enumerate(start_time_values, start=1):
    ...
    np.savetxt(f"new_site5/{start_time}_{index}.dat", optimized_data, delimiter="\t")

residual_data = np.column_stack((start_time_values,residual_sum))
final_pop = np.column_stack((start_time_values,final_pop))
np.savetxt(f"final_pop.dat",final_pop, delimiter = "\t")
np.savetxt(f"site_residual5.dat", residual_data,delimiter="\t")