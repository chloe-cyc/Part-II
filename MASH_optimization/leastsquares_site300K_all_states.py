...
column_names = ["Time"]
excitation_data_path = "/u/dem/kebl6911/Part-II/mash_site300K.dat"
for i in range(1,8):
...
def matkappa_to_matr(kappa):
    ...
    for h in range (no_states): #let matrix ij be also written as jh to avoid clashing in the same defined equation
        matr[h][h] = -sum(matr[j][h] for j in range(no_states) if j != h) 
    return matr
...
np.savetxt("site_ls.dat",p_least_squares, delimiter="\t")
end_time = time.time()
