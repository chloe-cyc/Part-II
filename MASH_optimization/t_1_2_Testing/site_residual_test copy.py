...
site_data = site_values[site_values[:,0]>=400]
for index, start_time in enumerate(start_time_values, start=1):
    ...
    prev_kap_1,calc_final = optimize(site_data, prev_kap, initial_guess_p, eq_pop, full_time,start_time) #t_one,t_two, full_time)
    prev_kap = prev_kap_1
...
np.savetxt(f"/u/dem/kebl6911/Part-II/MASH_optimization/t_1_2_Testing/t1_slide_2.dat",final_pop, delimiter = "\t")
