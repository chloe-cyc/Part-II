...
k = torch.tensor([0.01], requires_grad=True, dtype=torch.float32)

# DEFINE LOSS FUNCTION
def loss_function(actual_population, modelled_population):
    return torch.sum(((population_data -p_model_result)**2) for i in range())

#INITIALIZE ADAM with K:
optimizer = torch.optim.Adam([k], lr = 0.1)

#TRAINING LOOP
no_epochs =10

for epoch in range(no_epochs):
    model_populationmodel =   # Pass 'k' to your model to compute Pmodel(k)
    
    # Compute the loss
    loss = loss_function(Pideal, Pmodel)
    ...
    # Print the loss or other relevant information
